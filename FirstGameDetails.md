...
- 当前核心系统：
  ...
  - 剧情序列：`StoryTrigger` + `StorySequenceRunner` + `StorySequence` + `StoryStepBehaviour` + `StoryContext`。
...
- 函数：
  - `Update()`：每帧刷新 `IsGrounded`。
  - `DetectedGround()`：从 `point` 向下发射射线，命中地面层则返回 true。
  - `OnDrawGizmos()`：在 Scene 视图中绘制检测射线，方便调试检测距离。
- 关联：`PlayerGround`、`PlayerAir`、`Player_Fall` 等玩家状态通过 `groundSensor.IsGrounded` 决定落地、起跳、下落转换。
...
- 关联：`StorySequenceRunner` 创建它并传给每个 `StoryStepBehaviour`。

#### `Assets/_Game/Scripts/Runtime/GamePlay/Story/StoryStepBehaviour.cs`

- 脚本职责：剧情步骤基类。
- 函数：
  - `Execute(StoryContext context)`：由子类实现具体步骤协程。
- 关联：`StorySequence` 按列表保存它，`StorySequenceRunner` 顺序执行。
...
- 关键字段/属性：
  - `layerStack`、`flagCenter`、`questManager`：传入步骤上下文。
...
- 关联：`StoryTrigger` 触发它；步骤可使用上下文访问 Flag、Quest 和层栈。
...
- 函数：
  - `Awake()`：补齐序列运行器引用。
  - `OnTriggerEnter2D(Collider2D collision)`：进入触发器时尝试运行剧情。
- 关联：把物理触发区域和 `StorySequenceRunner` 连接起来。
...
#### `Assets/_Game/Scripts/Runtime/GamePlay/Story/Steps/WaitSecondsStoryStep.cs`

- 脚本职责：剧情等待步骤。
- 字段：
  - `duration`：等待秒数。
- 函数：
  - `Execute(StoryContext context)`：等待指定时间。
...
- 关键字段/属性：
  ...
  - `idleState`、`runState`、`runTurnState`、`runEndState`、`jumpStartState`、`jumpUpState`、`apexState`、`fallState`：所有玩家状态实例。
- 函数：
...
- 函数：
  ...
  - `ChangeStateToMoveState()`：根据移动输入切换到跑步或待机。
...
- 函数：
  ...
  - `LogicalUpdate()`：处理跳跃、世界交互、移动/待机/跑步转换；若离地则进入下落。
- 关联：`Player_IdleState`、`PlayerWalkState`、跑步相关状态继承或间接使用地面逻辑。
...
- 脚本职责：玩家步行状态，目前只有构造函数，实际逻辑主要继承 `PlayerGround`。
- 函数：
  - `PlayerWalkState(...)`：调用地面状态构造。
- 关联：预留给步行状态扩展；当前玩家更主要使用 Idle 和 Run。
...
- 函数：
  ...
  - `LogicalUpdate()`：若落地则处理跑步方向、转身、结束等逻辑；若离地则进入下落。
...
- 函数：
  ...
  - `LogicalUpdate()`：处理跳跃、松手缓冲、跑步结束、反向转身等状态转换。
...
3. `StorySequenceRunner` 创建 `StoryContext`。
4. `StorySequenceRunner` 顺序执行 `StorySequence.Steps` 中每个 `StoryStepBehaviour.Execute()`。
5. 当前已有 `DebugLogStoryStep` 和 `WaitSecondsStoryStep`，后续可扩展设置 Flag、改 Quest、切层、播放对话等步骤。
...
当前已经移除旧的 `Player_JumpState.cs`，跳跃拆分为 `JumpStart`、`JumpUp`、`Apex`、`Fall`。跑步逻辑集中在 `Player_Run` 子目录。
